...
router.post("/", (req, res, next) => {
    ...
    try {
        ...
        if (!author || !country || !imageLink || !language || !link || !pages || !title || !year) {
        ...
        //post processing
        const newBook = {
            author, country, imageLink, language, link, pages: parseInt(pages) || 1, title, year: parseInt(year) || 0, id: crypto.randomBytes(4).toString("hex")
        };
        //Read data from db.json then parse to JSobject
        let db = fs.readFileSync("db.json", "utf-8");
        db = JSON.parse(db);
